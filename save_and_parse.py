#!/usr/bin/env python3
"""
Script to save current Chrome page and parse it.
Uses AppleScript to get the page source from the active Chrome tab.
"""
...
def get_chrome_page_source():
    """Get HTML source from the current Chrome tab using AppleScript."""
    
    # AppleScript to get page source from Chrome
    applescript = '''
    tell application "Google Chrome"
        if (count of windows) > 0 then
            ...
            set pageSource to execute activeTab javascript "document.documentElement.outerHTML"
            return pageURL & "|||" & pageTitle & "|||" & pageSource
        else
            return "ERROR: No Chrome window open"
        end if
    end tell
    '''
    ...
    try:
        ...
        raise RuntimeError(f"Failed to get Chrome page: {e.stderr}")
...
def extract_content(html, url, method='auto'):
    ...
    if method in ['beautifulsoup', 'auto']:
        ...
        if not main_content:
            # Get body if no specific content area found
...
def main():
    try:
        print("Getting page from Chrome...", file=sys.stderr)
        url, title, html = get_chrome_page_source()
